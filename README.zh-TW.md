...
[`examples/`](./examples) 下有可直接執行的 Vue 與 React demo,各自有 `base` 與 `client-a` 兩個品牌(後者覆蓋了 `Banner`,並透過 `extends` 沿用 `Footer`)— 是上方 Quick start 的實作版本。

```bash
pnpm install
cd examples/vue   # 或 examples/react
pnpm dev
pnpm brand switch client-a && pnpm dev   # 觀察品牌切換效果
pnpm brand build base client-a           # 批次打包兩個品牌
```
